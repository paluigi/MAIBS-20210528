...
def testLiveData2():
    ...
    df_test["_C-earliest_cr_line_USA"] = pd.to_datetime(
        df_test["_C-earliest_cr_line_USA"]
    )
    df_test["_C-issue_d_USA"] = pd.to_datetime(df_test["_C-issue_d_USA"])
    df_test["time_delta"] = (
        df_test["_C-issue_d_USA"] - df_test["_C-earliest_cr_line_USA"]
    )
    ## All back to numeric
    df_test["_C-earliest_cr_line_USA"] = pd.to_numeric(
        df_test["_C-earliest_cr_line_USA"]
    )
    ...
    grade_cat = pd.Categorical(
        df_test["_C-sub_grade_USA"], categories=sub_grades, ordered=True
    )
