...
def trainModelOnCollectedData(df_train, path_files_ml):
  ...
  print('## Class balance ratio: ' + str(df_minority.shape[0] / df_majority_downsampled.shape[0]))
  ...
  df_balanced.drop(['id_USA', '_C-member_id_USA', '_C-default_ind'], axis=1, inplace=True)
  ...
  featureSel = feature_selection.SelectKBest(feature_selection.mutual_info_classif, k=10).fit(X, y)
...
def trainModel2(df_train, path_files_ml):
  ...
  df_balanced["_C-earliest_cr_line_USA"] = pd.to_datetime(df_balanced["_C-earliest_cr_line_USA"])
  df_balanced["_C-issue_d_USA"] = pd.to_datetime(df_balanced["_C-issue_d_USA"])
  df_balanced["time_delta"] = df_balanced["_C-issue_d_USA"] - df_balanced["_C-earliest_cr_line_USA"]
  ## All back to numeric
  df_balanced["_C-earliest_cr_line_USA"] = pd.to_numeric(df_balanced["_C-earliest_cr_line_USA"])
  ...
  sub_grades.sort()
  grade_cat = pd.Categorical(df_balanced["_C-sub_grade_USA"], categories=sub_grades, ordered=True)
  ...
  clf.fit(X, y)
  print(clf.cv_results_)
  n_est_best = clf.cv_results_['param_n_estimators'][clf.cv_results_['rank_test_score'][0] - 1]
  max_d_best = clf.cv_results_['param_max_depth'][clf.cv_results_['rank_test_score'][0] - 1]
  rfc_best = RandomForestClassifier(n_estimators=n_est_best,max_depth=max_d_best).fit(X, y)
